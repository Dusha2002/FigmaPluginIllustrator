...
1. Добавить профилирование/мониторинг для Railway-деплоя.
2. Реализовать экспорт PDF/A (приоритетно для архивов) и дополнительные проверки preflight.
3. Расширить поддержку TIFF-сжатий (ZIP/Deflate/JPEG) и параметры качества.
4. Улучшить UI плагина (кастомный сервер, индикаторы прогресса, предпросмотр).
5. Добавить е2е-тесты с Figma API / моком.
